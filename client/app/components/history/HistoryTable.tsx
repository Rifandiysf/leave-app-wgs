                        ...
                        {isLoading ? (
                            Array.from({ length: itemsPerPage }).map((_, rowIdx) => (
                                <tr key={rowIdx} className="animate-pulse">
                                    {Array.from({ length: 7 }).map((_, colIdx) => (
                                        <td key={colIdx} className="p-4 text-center border-b-[1.5px] border-[#0000001f]">
                            ...
                            data.map((item) => (
                                <tr key={item.id_leave} className="odd:bg-[#e8efff] even:bg-[#f8faff] hover:bg-[#e3e7f0] odd:dark:bg-card/30 even:dark:bg-card/70 transition-colors duration-300">
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{statusTag(item.status)}</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{formatUppercase(item.leave_type)}</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{formatDate(item.start_date)}</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{formatDate(item.end_date)}</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{item.total_days} Days</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">{item.title}</td>
                                    <td className="p-2 text-[14px] font-medium text-center border-b-[1.5px] border-[#0000001f]">
                                        <Modal
                                            mode="info"
                                            size="icon"
                                            variant="ghost"
